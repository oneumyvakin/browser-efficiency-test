...
// Returns pre-generated certificate from gentls folder
func LoadGeneratedCertificate(serverName string) ([]byte, string, error) {
	...
	folderName := "gentls"
	certPath := filepath.Join(cwd, folderName, serverName + ".crt")
	...
	if certBlock == nil {
		return nil, "", fmt.Errorf("empty pem.Decode for '%s'", certPath)
	}
...
// Returns pre-generated certificate from gentls folder
func LoadGeneratedPrivateKey(serverName string) (crypto.PrivateKey, error) {
	cwd, err := osext.ExecutableFolder()
	if err != nil {
		return nil, err
	}
	folderName := "gentls"
	keyPath := filepath.Join(cwd, folderName, serverName + ".key")
	...
	if keyBlock == nil {
		return nil, fmt.Errorf("empty pem.Decode for '%s'", keyPath)
	}
...
func (tp *tlsProxy) getReplayConfigForClient(clientHello *tls.ClientHelloInfo) (*tls.Config, error) {
	...
	configPrivateKey, err := LoadGeneratedPrivateKey(h)
	if err != nil {
		configPrivateKey = tp.root.PrivateKey
	}

	if err != nil || derBytes == nil {
		if _, ok := tp.dummy_certs_map[h]; !ok {
			derBytes, negotiatedProtocol, err = LoadGeneratedCertificate(h)
			if err != nil {
				return nil, err
			}
